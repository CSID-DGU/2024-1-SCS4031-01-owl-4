...
import ThirdSettingForm from "../../../components/ThirdSettingForm.jsx";
...
const formSchema = z.object({
  ...
  stop_loss_point: z.preprocess(
    ...
    z
      .number()
      .positive({ message: "Stop loss point must be a positive number" })
  ),
});
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      ...
      const response = await axios.post(
        "http://localhost:8081/api/v1/backtesting/run",
        data,
        {
          headers: {
    ...
    onSuccess: () => {
      ...
      setMdate("60"),
      setNdate("20"),
      setBuyingCondition(5),
      setSellingCondition(3),
      setBuyingSplit(20),
      setStopLossPoint(10);
    ...
    <>
      <div className="basis-[50px] rounded-t-xl flex relative">
        ...
        <div className="grow-[1] flex justify-center items-center rounded-l-xl shadow-[-5px_5px_0_rgb(255,255,255)]">
          <span className="block w-2/3 bg-orange-500 font-bold text-white text-xl text-center rounded-xl">
